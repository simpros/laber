...
/**
 * One definition of "core is configured": every required key non-empty, so a
 * lone ROOT_DOMAIN never reports configured when deploy would still 400.
 */
...
/** "Core configured + services" assembly shared by `GET /api/core` and the dashboard. */
...
export async function deployCore() {
  ...
  // The generated bytes travel as `deploy.composeBytes` and go live via `commitLive` in the same attempt.
