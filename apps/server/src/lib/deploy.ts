...
type DeployOptions = {
  composePath: string;
  /**
   * The exact compose bytes Docker applies. When set, `deployStack` owns the
   * snapshot lifecycle: it freezes the bytes to a sibling temp file in the
   * same directory (same `cwd` + relative-path resolution as the live file),
   ...
   * with its own write/rm block. When absent, `composePath` is used directly.
   */
  composeBytes?: string;
...
export async function deployStack(
  ...
  try {
    if (options.composeBytes !== undefined) {
